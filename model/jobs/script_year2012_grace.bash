...
##PBS -q devel
#PBS -q long
#PBS -o smartosse_year_grace.out
#PBS -e smartosse_year_grace.err
...
builddir=$basedir/build_cost_genocst_bpv4_mo
#build${whichexp}
inputdir=$basedir/input_labsea_daily
#rundir=run${mitgcm_version}${whichexp}_natl_1month_alldailyxx_gracellc4320_spread
rundir=run${mitgcm_version}${whichexp}_natl_1month_alldailyxx_gracellc4320_stdold

workdir_root_root_root=$scratchdir/osses/${rundir}

optimdir=${basedir}/OPTIM_fullyear

#==================================================
# REGIONS
#==================================================

regions=("labsea" "newfoundland" "northsea" "subgyre")
nss=(64 27 41 25)
regions=("fullnatl")
nss=(142)
...
#--------------------------------------------------
# Find next region-iteration task
#--------------------------------------------------
...
find_next_task () {
    ...
    for t in "${!pickup_tss[@]}"; do
        ...
        for r in "${!regions[@]}"; do
            ...
            region=${regions[r]}
            workdir_root=$workdir_root_root/$region
...
while true; do
    ...
    read t r iter <<< $(find_next_task)

    if [[ $t -lt 0 ]]; then
        echo "ALL REGIONS COMPLETE → exiting"
        break
    fi
    ...
    echo "======================================="
    echo "RUNNING: $region  iter${optimext}"
    echo "======================================="
    ...
    rm -f tapes/* profiles/*
    cp $basedir/script_year2012.bash .
    cp -rf ${codedir}/ .
    ...
    ln -s ${scratchdir}/run_template//input_ecco/smart_phibot/grace_llc4320/$aste_bp_fname.bin ./$aste_bp_fname
...
#    sed -i "/gencost_barfile(1)/s/mon/day/" data.ecco
    ...
    cp data.ctrl_dailyxx_multgen_jrastdapressure data.ctrl
